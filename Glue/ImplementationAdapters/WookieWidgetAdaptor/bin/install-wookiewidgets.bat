...
:: Registers WookieWidgetsAdapter as a Windows service

@ECHO ** Running install-wookiewidgets...
@ECHO OFF
...
SET WOOKIE_LIB=%ADAPTER_LIB_DIR%\dep\wookie-connector-framework\wookie-connector-framework-20100601.jar
SET WOOKIE_EXT_LIB=%ADAPTER_LIB_DIR%\dep\slf4j\slf4j-api-1.6.0.jar;%ADAPTER_LIB_DIR%\dep\slf4j\slf4j-simple-1.6.0.jar

:: Classpath composition
SET CLASSPATH=%CONF_DIR%;%ADAPTER_LIB_DIR%\wookiewidgets-adapter.jar;%GLUE_LIB_DIR%\glue-common.jar;%RESTLET_DIR%\org.restlet.jar;%RESTLET_DIR%\org.restlet.ext.xml.jar;%RESTLET_DIR%\org.restlet.ext.atom.jar;%WOOKIE_LIB%;%WOOKIE_EXT_LIB%;%RESTLET_JETTY_CONN%

:: Go!
SET MANUAL_MODE=%1
IF NOT DEFINED MANUAL_MODE GOTO auto
IF %MANUAL_MODE% NEQ manual GOTO auto
%GLUE_HOME%\bin\ServiceInstaller //IS//WookieWidgetsAdapter --DisplayName=WookieWidgetsAdapter --Startup=manual --Classpath=%CLASSPATH% --StartMode=Java --StartClass=glue.adapters.implementation.wookiewidgets.manager.WookieWidgetsAdapterServerMain --StartPath=%DATA_DIR% --LogPath=%COMMONS_LOG_DIR% --LogPrefix=%ADAPTER_KEY%_adapter --StdOutput=%LOG_DIR%\%ADAPTER_KEY%_adapter.log --StdError=%LOG_DIR%\%ADAPTER_KEY%_adapter.log
EXIT /B

:auto
%GLUE_HOME%\bin\ServiceInstaller //IS//WookieWidgetsAdapter --DisplayName=WookieWidgetsAdapter --Startup=auto --Classpath=%CLASSPATH% --StartMode=Java --StartClass=glue.adapters.implementation.wookiewidgets.manager.WookieWidgetsAdapterServerMain --StartPath=%DATA_DIR% --LogPath=%COMMONS_LOG_DIR% --LogPrefix=%ADAPTER_KEY%_adapter --StdOutput=%LOG_DIR%\%ADAPTER_KEY%_adapter.log --StdError=%LOG_DIR%\%ADAPTER_KEY%_adapter.log

...
// TODO: Check... are you sure you meant to do this Ben? :)	
	...
	/**
	 * Get the metadata where the entities they are referring to match a given criteria.
	 * But not taking into account the entity_guid at all.
	 * 
	 * @param mixed $meta_name 
	 * @param mixed $meta_value
	 * @param string $entity_type The type of entity to look for, eg 'site' or 'object'
	 ...
	 * @param int $site_guid The site to get entities for. Leave as 0 (default) for the current site; -1 for all sites.
	 */
	function find_metadata_noentity($meta_name = "", $meta_value = "", $entity_type = "", $entity_subtype = "", $limit = 10, $offset = 0, $order_by = "")
	{
		global $CONFIG;
...
//echo $query;
	...
	/**
	 * Get the metadata where the entities they are referring to match a given criteria.
	 * Capable of finding string parts
	 * 
	 * @param mixed $meta_name 
	 * @param mixed $meta_value
	 * @param string $entity_type The type of entity to look for, eg 'site' or 'object'
	 ...
	 * @param string $order_by Optional ordering.
	 */
	function find_metadata_like($meta_name = "", $meta_value = "", $entity_type = "", $entity_subtype = "", $limit = 10, $offset = 0, $order_by = "")
	{
		global $CONFIG;
		...
		if ($entity_subtype)
			$where[] = "e.subtype=$entity_subtype";
		if ($meta_name!="") {
			if (!$meta_n) return false; // The value is set, but we didn't get a value... so something went wrong.
			$where[] = "m.name_id='$meta_n'";
		}
		if ($meta_value!="") {
			$where[] = "v.string LIKE '%$meta_value%'";
		}
		
		$query = "SELECT DISTINCT v.string as value, m.value_id as value_id

...
XML;
    ...
    public function saveNewDocument()
    {
        ...
        $filestorename = 'lds_'.(string)$rand_id;
        $file = $this->getNewFile($filestorename);
...
SQL;
    ...
    private function schedule_cache_remote_gdoc($data) {
        ...
        $cached_gdoc->description   = base64_encode(serialize($data));
    ...
    protected function saveGoogleExport($item, $google_file) {
        $item_to_mime = array(
            'pdf' => 'application/pdf',
        ...
        $mime = $item_to_mime[$item];

        if(empty($google_file->exportLinks[$mime]))
            return false;

        $result = ldshake_download_file($google_file->exportLinks[$mime], $path);
    ...
    protected function saveExportSpreadsheet($google_file) {
...
class MoodleManager
{
    ...
    private function moodleLogin() {
        ...
        $moodle_url = rtrim($this->moodle_url, '/');

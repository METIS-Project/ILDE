<?php
	 ...
	 */
		function elgg_view_comments($entity){
			...
			if ($comments = trigger_plugin_hook('comments',$entity->getType(),array('entity' => $entity),false)) {
				...
				/// LdShake change ///
	            $comments .= list_annotations($entity->getGUID(),'generic_comment',25,false);
				/// LdShake change ///	            
	...
	/// LdShake change ///
	//Make it more meaningful: avoid "532 days ago", which doesn't make sense
	function friendly_time ($tstamp, $usePrepisition = true, $short = false)
	{
		//Segundos de diferencia
		$difference = time() - $tstamp;
		...
		if ($difference / 60 < 60)
		{
			...
			if ($minutes == 0)
				...
				return elgg_echo("friendlytime:minutes:singular");
			else
				return sprintf(elgg_echo("friendlytime:minutes"),$minutes);
			
		}
		//Cuando hace menos de 7 horas (no incluida)
		elseif ($difference / 60 / 60 < 7 )
		{
			$hours = intval($difference / 60 / 60);
			
			if ($hours == 1)
				return elgg_echo("friendlytime:hours:singular");
			else
				return sprintf(elgg_echo("friendlytime:hours"),$hours);
				
		}
		elseif (date("Y", $tstamp) == date("Y") && date("m", $tstamp) == date("m") && date("d", $tstamp) == date("d"))
		{
			return elgg_echo("friendlytime:today");
		}
		elseif (date("Y", $tstamp) == date("Y") && date("m", $tstamp) == date("m") && date("d", $tstamp) == date("d") - 1)
		{
			return elgg_echo("friendlytime:days:singular");
		}
		else
		{
			$mFormat = $short ? 'M' : 'F';
			...
			if (date("Y", $tstamp) == date("Y"))
				return ($usePrepisition?'on ':'').date("j $mFormat", $tstamp);
			else
				return ($usePrepisition?'on ':'').date("j $mFormat $yFormat", $tstamp);
		}
		/// LdShake change ///
	}
	...
	function friendly_title($title)
	{
			...
			/// LdShake change ///
			//Reimplemented to support UTF-8 and to flatten accents as well. Should be a plugin hook in order not to
			//modify the ELGG core.
			/*
			$title = trim($title);
			...
			*/
			...
			$title = strtr ($title, $subst);//utf8_decode($uglyChars), $prettyChars);

<?php
	 ...
	 */
		...
		/// LdShake change ///		
		/**
		 * Get a specified access collection by its name.
		 * If a userId is specified, forces an ownerId
		 *
		 * @param int $collection_name The collection name
		 * @param int $ownerId The owner of the collection
		 * @return array|false Depending on success
		 */
		function get_access_collection_by_name($collection_name, $ownerId = null) {
    		
    		global $CONFIG;
    		
    		if (is_null($ownerId))
    			$get_collection = get_data_row("SELECT * FROM {$CONFIG->dbprefix}access_collections WHERE name = '{$collection_name}' LIMIT 1");
    		else
    			$get_collection = get_data_row("SELECT * FROM {$CONFIG->dbprefix}access_collections WHERE name = '{$collection_name}' AND owner_guid = '{$ownerId}' LIMIT 1");
		...
		function get_members_of_access_collection($collection, $idonly = false) {
    		...
    		if (!$idonly) {
    			...
    			if (is_array($collection_members))

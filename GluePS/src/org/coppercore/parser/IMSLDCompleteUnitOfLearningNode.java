...
/**
 * This class represent the IMS learning design complete-unit-of-learning element.
 ...
 */
public class IMSLDCompleteUnitOfLearningNode
    extends OnCompletion {
  private ArrayList playsToComplete = null;

  /**
   * Constructs a IMSLDCompleteUnitOfLearningNode instance from the passed xml dom element.
   * 
   ...
   */
  public IMSLDCompleteUnitOfLearningNode(Node aNode, IMSLDNode aParent) {
    super(aNode, aParent);
  }
  ...
  protected void parseLDElement(Element childNode, String nodeName) throws Exception {
    ...
    if (nodeName.equals("when-play-completed")) {
      //we cannot solve reference directly, so use reference for now
      addPlayReference(getNamedAttribute(childNode,"ref"));
    }
  }

  protected void resolveReferences() throws Exception {

    lookupAll(playsToComplete);

    super.resolveReferences();
  }

  private void addPlayReference(String ref) {
    if (playsToComplete == null) {
      playsToComplete = new ArrayList();
    }

    playsToComplete.add(ref);
  }



  CompletionComponentDef getComponentDef() {
    return ((IMSLDLearningDesignNode) parent.parent).getComponentDef();
  }
  ...
  protected boolean buildComponentModel(int uolId, MessageList messages) {
    ...
    if (playsToComplete != null) {
      condition = new Condition(buildPlaysToCompleteCondition(), actionList);
    }
  ...
  private IfExpression buildPlaysToCompleteCondition() {
    //build the condition
    ...
    Iterator iter = playsToComplete.iterator();
    while (iter.hasNext()) {
      IMSLDPlayNode play = (IMSLDPlayNode) iter.next();
      completeExpression = new Complete(play.getIdentifier(),
                                        play.getNameForCompleteExpression());

      andExpression.addOperand(completeExpression);
    }
    if (playsToComplete.size() >= 2) {

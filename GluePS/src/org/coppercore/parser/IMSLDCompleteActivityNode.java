...
/**
 * 
 * This class represents an IMS learing design complete-activity element. 
 *
 * @author Harrie Martens
 * @author Hubert Vogten
 * @version $Revision: 1.11 $, $Date: 2005/06/16 22:09:56 $
 */
public class IMSLDCompleteActivityNode extends OnCompletion {
  private boolean userChoice = false;

  /**
   * Constructs a IMSLDCompleteActivityNode instance from the passed xml dom element.
   * 
   ...
   */
  public IMSLDCompleteActivityNode(Node aNode, IMSLDNode aParent) {
    super(aNode, aParent);
  }
  ...
  protected void parseLDElement(Element childNode, String nodeName) throws Exception {
    ...
    if (nodeName.equals("user-choice")) {
  ...
  /**
   * Returns the value of the user-choice attribute of the complete-activity element.
   * @return the value of the user-choice attribute of the complete-activity element
   */
  ...
  CompletionComponentDef getComponentDef() {
    return ((IMSLDLearningActivityNode) parent).getComponentDef();
  }
  ...
  protected boolean buildComponentModel(int uolId, MessageList messages) {
    ...
    CompletionComponentDef componentDef = null;

    //create the SupportActivityDef now

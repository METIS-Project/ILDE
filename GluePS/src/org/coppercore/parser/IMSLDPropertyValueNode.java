...
/**
 * 
 * This class represents an IMS learning design property-value element.  
 *
 * @author Harrie Martens
 * @author Hubert Vogten
 * @version $Revision: 1.11 $, $Date: 2005/01/21 14:01:10 $
 */
public class IMSLDPropertyValueNode
    extends OperandEntity {
  private IMSLDNode content = null;

  /**
   * Constructs a IMSLDPropertyValueNode instance from the passed xml dom element.
   * 
   ...
   */
  public IMSLDPropertyValueNode(Node aNode, IMSLDNode aParent) {
    super(aNode, aParent);
  }

  protected void parseLDElement(Element childNode, String nodeName) throws Exception {
    //this deals with property-ref
    super.parseLDElement(childNode, nodeName);

    if (nodeName.equals("langstring")) {
      content = addElement(new IMSLDLangStringNode(childNode, this));
    }
    else if (nodeName.equals("calculate")) {
      content = addElement(new IMSLDCalculateNode(childNode, this));
    }
  }
  ...
  protected Operand getExpression() throws ValidationException {
    ...
    if (content != null) {
      ...
      PropertyNode property = (PropertyNode) properties.get(0);
      return new Property(property.getPropertyDef());
    }

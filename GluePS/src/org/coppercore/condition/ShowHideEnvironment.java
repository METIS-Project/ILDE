...
import org.coppercore.component.EnvironmentProperty;
import org.coppercore.exceptions.ExpressionException;
import org.coppercore.exceptions.PropertyException;
import org.coppercore.exceptions.TypeCastException;
import org.w3c.dom.Element;

public class ShowHideEnvironment extends ShowHide{
  private static final long serialVersionUID = 42L;
  public ShowHideEnvironment(String identifier, boolean show) {
    super(identifier,show);
  }

  public static ShowHideEnvironment create(Element node) {
    boolean show = "true".equalsIgnoreCase(node.getAttribute("show"));
    String id = node.getAttribute("ref");
    return new ShowHideEnvironment(id,show);
  ...
  protected void performAction(Uol uol, Run run, User user, Collection firedActions) throws TypeCastException, PropertyException, ExpressionException {
    EnvironmentProperty environment = ComponentFactory.getPropertyFactory().
        getEnvironment(uol, run, user, identifier);
    environment.setVisibility(show);
  }



  protected void toXml(PrintWriter out) {
    toXml(out,"show-hide-environment");
  }
}

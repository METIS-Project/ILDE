...
public class ShowHideUnitOfLearning extends ShowHide {
  private static final long serialVersionUID = 42L;
  public ShowHideUnitOfLearning(String identifier, boolean show) {
    super(identifier, show);
  }

  public static ShowHideUnitOfLearning create(Element node) {
    boolean show = "true".equalsIgnoreCase(node.getAttribute("show"));
    String id = node.getAttribute("ref");
    return new ShowHideUnitOfLearning(id, show);
  }

  protected void performAction(Uol uol, Run run, User user, Collection firedActions) throws TypeCastException,
      PropertyException, ExpressionException {
    UnitOfLearningProperty unitOfLearning = ComponentFactory.getPropertyFactory().getUnitOfLearning(uol, run, user,
        identifier);
    unitOfLearning.setVisibility(show);
  }

  protected void toXml(PrintWriter out) {
    toXml(out, "show-hide-unit-of-learning");
  }
}
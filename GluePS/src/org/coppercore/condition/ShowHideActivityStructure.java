...
public class ShowHideActivityStructure extends ShowHide {
  private static final long serialVersionUID = 42L;

  public ShowHideActivityStructure(String identifier, boolean show) {
    super(identifier,show);
  }

  public static ShowHideActivityStructure create(Element node) {
    boolean show = "true".equalsIgnoreCase(node.getAttribute("show"));
    String id = node.getAttribute("ref");
    return new ShowHideActivityStructure(id,show);
  }
  ...
  protected void performAction(Uol uol, Run run, User user, Collection firedActions) throws TypeCastException, PropertyException, ExpressionException {
    ActivityStructureProperty activityStructure = ComponentFactory.getPropertyFactory().getActivityStructure(uol,run,user,identifier);
    activityStructure.setVisibility(show);
  }


  protected void toXml(PrintWriter out) {
    toXml(out,"show-hide-activity-structure");
  }
}

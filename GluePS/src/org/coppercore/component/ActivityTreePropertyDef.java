...
/**
 * This class represents the activity treedefinition. An activity corresponds
 * with the plays and its children defined in IMS LD, filetered on a particular
 * role.
 *
 * @author Harrie Martens
 * @author Hubert Vogten
 * @version $Revision: 1.18 $, $Date: 2005/01/21 10:58:48 $
 */
public class ActivityTreePropertyDef extends PropertyDef {
  private static final long serialVersionUID = 1L;

  /**
   * data type of this component.
   */
  public final static String DATATYPE = "activity-tree";

  /**
   * defines the prefix used for defining the unique database id of this component.
   */
  ...
  private String activityTreeString = null;
   ...
   */
  public ActivityTreePropertyDef(int uolId, String propId) throws PropertyException, TypeCastException {
    super(uolId, propId);
  }
   ...
   * @param activityTree
   *          String the XML representation of the activity tree
   */
  public ActivityTreePropertyDef(int uolId, String propId, String activityTree) {

    this.uolId = uolId;
    //add the prefix to the propId ensuring that it is unique within the uol
    this.propId = IDPREFIX + propId;
    this.activityTreeString = activityTree;
  ...
  protected boolean processElement(Element node, int anUolId) throws PropertyException {
    ...
    if (node.getNodeName().equals("value")) {
      activityTree = (Element) node.getFirstChild();
      return true;
    }
    return false;
  }
  ...
  protected void toXml(PrintWriter result) {
    result.write("<value>" + activityTreeString + "</value>");
  }
  ...
  protected Element getTree() {
    //clone the node to protect the defintion from being modified during personalization
    return (Element) activityTree.cloneNode(true);
  }
}
...
public class LessThanExpression extends Expression {
  ...
  public LDDataType evaluate(Uol uol, Run run, User user)
    ...
    if (operands.size() == 2) {
      ...
      return opOne.evaluate(uol, run, user).lessThan(
  ...
  public int checkType() throws ExpressionException {
    ...
    if (operands.size() == 2) {
      ...
      case LDDataType.LDINTEGER: {
        return LDInteger.lessThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDSTRING: {
        return LDString.lessThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDBOOLEAN: {
        return LDBoolean.lessThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDREAL: {
        return LDReal.lessThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDATETIME: {
        return LDDateTime.lessThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDURATION: {
        return LDDuration.lessThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDURI: {
        return LDUri.lessThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDCONSTANT: {
        //apply sum method directly on constant as we have the object during
        // parse time
        return ((PropertyConstant) opOne).getValue().lessThan(opTwo);
      }
      default: {
  ...
  public void toXml(PrintWriter out) {
    XMLTag tag = new XMLTag(Operand.LESSTHAN);

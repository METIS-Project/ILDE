...
import org.coppercore.datatypes.LDText;
...
public class IsExpression extends Expression {
  ...
  public int checkType() throws ExpressionException {
    ...
    if (operands.size() == 2) {
      ...
      case LDDataType.LDINTEGER: {
        return LDInteger.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDSTRING: {
        return LDString.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDTEXT: {
        return LDText.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDBOOLEAN: {
        return LDBoolean.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDREAL: {
        return LDReal.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDATETIME: {
        return LDDateTime.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDURATION: {
        return LDDuration.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDURI: {
        return LDUri.is(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDCONSTANT: {
        //apply sum method directly on constant as we have the object during
        // parse time
        return ((PropertyConstant) opOne).getValue().is(opTwo);
  ...
  public LDDataType evaluate(Uol uol, Run run, User user)
    ...
    if (operands.size() == 2) {
      ...
      return opOne.evaluate(uol, run, user).is(opTwo.evaluate(uol, run, user));
    }
    throw new IllegalOperandException(
        "Incorrect number of operands encountered");
  }
  ...
  public void toXml(PrintWriter out) {
    XMLTag tag = new XMLTag(Operand.IS);

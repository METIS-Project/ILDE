...
public class DivideExpression extends Expression {
  ...
  public LDDataType evaluate(Uol uol, Run run, User user)
    ...
    if (operands.size() == 2) {
      ...
      return opOne.evaluate(uol, run, user).divide(
  ...
  public int checkType() throws ExpressionException {
    ...
    if (operands.size() == 2) {
      ...
      case LDDataType.LDINTEGER: {
        return LDInteger.divide(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDSTRING: {
        return LDString.divide(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDBOOLEAN: {
        return LDBoolean.divide(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDREAL: {
        return LDReal.divide(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDATETIME: {
        return LDDateTime.divide(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDURATION: {
        return LDDuration.divide(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDURI: {
        return LDUri.divide(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDCONSTANT: {
        //apply sum method directly on constant as we have the object during
        // parse time
        return ((PropertyConstant) opOne).getValue().divide(opTwo);
      }
      default: {
  ...
  public void toXml(PrintWriter out) {
    XMLTag tag = new XMLTag(Operand.DIVIDE);

...
public class MultiplyExpression extends Expression {
  ...
  public LDDataType evaluate(Uol uol, Run run, User user)
    ...
    if (operands.size() == 2) {
      ...
      return opOne.evaluate(uol, run, user).multiply(
  ...
  public int checkType() throws ExpressionException {
    ...
    if (operands.size() == 2) {
      ...
      case LDDataType.LDINTEGER: {
        return LDInteger.multiply(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDSTRING: {
        return LDString.multiply(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDBOOLEAN: {
        return LDBoolean.multiply(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDREAL: {
        return LDReal.multiply(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDATETIME: {
        return LDDateTime.multiply(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDURATION: {
        return LDDuration.multiply(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDURI: {
        return LDUri.multiply(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDCONSTANT: {
        //apply sum method directly on constant as we have the object during
        // parse time
        return ((PropertyConstant) opOne).getValue().multiply(opTwo);
      }
      default: {
  ...
  public void toXml(PrintWriter out) {
    XMLTag tag = new XMLTag(Operand.MULTIPLY);

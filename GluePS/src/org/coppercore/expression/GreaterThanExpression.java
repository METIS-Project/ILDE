...
public class GreaterThanExpression extends Expression {
  ...
  public LDDataType evaluate(Uol uol, Run run, User user)
    ...
    if (operands.size() == 2) {
      ...
      return opOne.evaluate(uol, run, user).greaterThan(
  ...
  public int checkType() throws ExpressionException {
    ...
    if (operands.size() == 2) {
      ...
      case LDDataType.LDINTEGER: {
        return LDInteger.greaterThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDSTRING: {
        return LDString.greaterThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDBOOLEAN: {
        return LDBoolean.greaterThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDREAL: {
        return LDReal.greaterThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDATETIME: {
        return LDDateTime.greaterThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDURATION: {
        return LDDuration.greaterThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDURI: {
        return LDUri.greaterThan(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDCONSTANT: {
        //apply sum method directly on constant as we have the object during
        // parse time
        return ((PropertyConstant) opOne).getValue().greaterThan(opTwo);
      }
      default: {
  ...
  public void toXml(PrintWriter out) {
    XMLTag tag = new XMLTag(Operand.GREATERTHAN);

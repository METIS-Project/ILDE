...
public class SumExpression extends Expression {
  ...
  public LDDataType evaluate(Uol uol, Run run, User user) throws PropertyException, ExpressionException {
    ...
    if (operands.size() == 2) {
      ...
      return opOne.evaluate(uol, run, user).sum(opTwo.evaluate(uol, run, user));
    }
  ...
  public int checkType() throws ExpressionException {
    ...
    if (operands.size() == 2) {
      ...
      case LDDataType.LDINTEGER: {
        return LDInteger.sum(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDSTRING: {
        return LDString.sum(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDBOOLEAN: {
        return LDBoolean.sum(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDREAL: {
        return LDReal.sum(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDATETIME: {
        return LDDateTime.sum(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDDURATION: {
        return LDDuration.sum(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDURI: {
        return LDUri.sum(opTwo.checkType(), opTwo);
      }
      case LDDataType.LDCONSTANT: {
        //apply sum method directly on constant as we have the object during
        // parse time
        return ((PropertyConstant) opOne).getValue().sum(opTwo);
      }
      default: {
  ...
  public void toXml(PrintWriter out) {
    XMLTag tag = new XMLTag(Operand.SUM);

...
import zipfile
from exe.ldshake.fileupload     import UploadFolder
...
class MainPage(RenderableLivePage):
    ...
    def __init__(self, parent, package, session, config):
        ...
        self.putChild("upload", UploadFolder())
		#styles directory
...
#        inevow.IRequest(ctx).setHeader('content-type', 'application/vnd.mozilla.xul+xml')
        ...
        #LdShake: the server is always on
    ...
    def render_config(self, ctx, data):
        config = {'lastDir': G.application.config.lastDir,
                  ...
                  'showPreferences': G.application.config.showPreferencesOnStart == '0' and not G.application.preferencesShowed,
...
#        data.close()
    ...
    def handleInsertPackage(self, client, filename):
        ...
        tmpfolder = G.application.webServer.getSessionFolder(uid)
        filename = unicode(str(tmpfolder), 'utf-8') + u'/' + filename
        package = self._loadPackage(client, filename, newLoad=True)

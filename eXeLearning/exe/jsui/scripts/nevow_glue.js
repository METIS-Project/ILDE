...
ldshake_input_semaphore = false;
ldshake_input_clean_semaphore = false;
...
function connect(outputNum) {
    ...
    if(ldshake_input_clean_semaphore){
        clean_input = true;
    }
...
function nevow_clientToServerEvent(theTarget, node, evalAfterDone) {
    if(ldshake_input_semaphore)
        return false;

    ldshake_input_semaphore = true;
    ...
    input.onreadystatechange = function() {
        ...
        if (input.readyState == 4) {
	        ldshake_input_clean_semaphore = true;
        }
    }
    input.open(
      "GET",

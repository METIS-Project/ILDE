...
class PackageRedirectPage(RenderableResource):
    ...
    def bindNewPackage(self, package, session):
        ...
        return self.mainpages.get(session.uid)[package.name]

    # LdShake mod
    def nametouid(self, name):
        if name in self.packagesession:
            return self.packagesession[name]
    ...
    def render_GET(self, request):
        ...
        self.webServer.getSessionFolder(session.uid)
        self.packagesession[package.name] = session.uid

...
class Element(object):
    ...
    def getTmpFolder(self):
        package = self.field.idevice.parentNode.package
        root = G.application.webServer.root
        uid = root.nametouid(package.name)
...
class ImageElement(Element):
    ...
    def process(self, request):
        ...
        and not is_cancel:
            ...
            filePath = unicode(str(tmpfolder), 'utf-8') + u'/' + request.args["path"+self.id][0]
            self.field.setImage(filePath)
    ...
    def renderEdit(self):
        ...
        html += u"onclick=\"%s(event, '%s');\" " % (function, self.id)
        ...
        html += u'<input type="button" onclick="%s(event, \'%s\')"' % (function, self.id)
        html += u' value="%s" />' % _(u"Select an image")
        html += u'<input type="file" style="display:none" />'
...
class AttachmentElement(Element):
    ...
    def renderEdit(self):
        ...
        html += common.elementInstruc(self.field.instruc)+'<br/>'
        html += common.textInput("path"+self.id, "", 50)
        html += u'<input type="button" onclick="addFile(event, \'%s\')"' % self.id
        html += u' value="%s" /><br/>\n' % _(u"Select a file")
...
class MagnifierElement(Element):
    ...
    def process(self, request):
        ...
        self.field.message = ""
        if "path"+self.id in request.args:
            path = unicode(str(self.getTmpFolder()), 'utf-8') + u'/' + request.args["path"+self.id][0]
            """
    ...
    def renderEdit(self):
        ...
        html += u'<img alt="%s" ' % _('Add Image')
        html += u'id="img%s" ' % self.id
        html += u"onclick=\"addImage(event, '"+self.id+"');\" "
        ...
        html += u' onclick="addImage(event, \'%s\')"' % self.id
        html += u' value="%s" />' % _(u"Select an image")
        html += u'<input type="file" style="display:none" />'
        if self.field.imageResource and not self.field.isDefaultImage:
            html += '<p style="color: red;">'+ self.field.imageResource.storageName + '</P>'

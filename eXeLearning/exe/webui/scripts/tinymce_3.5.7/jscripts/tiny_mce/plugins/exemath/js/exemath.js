...
(function() {
	...
	window.ExeMath = {
		...
		createMathImage : function(field_name, src_latex, font_size, type, win) {
			...
			var w = "";
			if (typeof(ldshake_top().nevow_clientToServerEvent)!='undefined') w = ldshake_top();
			else {
            ...
            w.latexcompleted = function() {
                ...
                // PreviewImage is only available for images:
                if (type == "image") {
                   mathobject.updateLatexImage(full_preview_url);
                }
            }

            w.nevow_clientToServerEvent('generateTinyMCEmath', this,
              'latexcompleted()', win, win.name, field_name,

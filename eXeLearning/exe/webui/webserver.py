...
from exe.ldshake.resteditor import LdshakeRestEditor
from exe.ldshake.ldshakegui import LdshakeGUI
from exe.ldshake.previewfolder import PreviewFolder
...
class WebServer:
    ...
    def run(self):
        ...
        self.root.putChild("previews",    PreviewFolder())
        ...
        # LdShake REST editor
        self.root.putChild("ldshake", Resource())
        ldshake = self.root.children.get("ldshake")
        ldshake.putChild("ldsdoc", LdshakeRestEditor(self))
        self.root.putChild("ldshakegui", LdshakeGUI(self))
    ...
    def getSessionFolder(self, uid):
        if uid not in self.sessiontmpdir:
            self.sessiontmpdir[uid] = TempDirPath()

        return self.sessiontmpdir[uid]